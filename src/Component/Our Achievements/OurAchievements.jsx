      ...
      <div className="bg-blue-50 py-12 px-4 sm:px-8 md:px-16 lg:px-24">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-12">
          ...
          <div className="grid grid-cols-2 gap-8 max-w-lg w-full">
            {achievements.map((item, index) => (
              <div key={index} className="flex flex-col items-start text-left">
                <div className="bg-red-100 text-red-500 w-12 h-12 rounded-full flex items-center justify-center text-2xl mb-3">
                  🏆
                </div>
                <div className="text-red-500 font-bold text-xl">{item.value}</div>
          ...
          <div className="relative">
            <img src={userMap} alt="World Map" className="w-[700px] h-[270px]" />
            {locations.map((loc, idx) => (
              ...
              >
                {loc.name}
                <div className="text-xs text-gray-500 font-normal">{loc.offices}</div>
              </div>
            ))}
          </div>
        </div>
