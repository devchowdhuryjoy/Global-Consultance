...
const offices = [
  ...
  {
    name: 'Banani',
    address: `RSR Tower (7th Floor), House no. 50, Block C,\nRd No. 11, Banani, Dhaka 1213`,
...
const Footer = () => {
  return (
    <div className="bg-[#f3f3f3] pt-10 pb-4 px-4 sm:px-8 md:px-16 lg:px-24">
      <h2 className="text-center text-xl sm:text-2xl font-semibold text-black mb-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 text-sm text-gray-800">
        {offices.map((office, idx) => (
          <div key={idx}>
            <h3 className="text-black font-semibold text-lg mb-2">{office.name}</h3>
            <p className="flex items-start gap-2 text-sm mb-2 whitespace-pre-line">
              <FaMapMarkerAlt className="text-pink-500 mt-1" /> {office.address}
            </p>
            <p className="flex items-center gap-2">
              <FaPhone className="text-red-600" /> {office.phone}
            </p>
          </div>
        ))}
      </div>
      ...
      <div className="mt-8 border-t pt-4 text-center text-sm text-gray-600">
        Copyright © 2025 <span className="font-medium">Global Routways Bangladesh</span> |
