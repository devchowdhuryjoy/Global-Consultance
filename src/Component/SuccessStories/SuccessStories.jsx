...
//                 {review.image && (
//                   <img src={review.image} alt="Success Story" className="w-24 mt-2" />
//                 )}
...
const SuccessStories = () => {
  ...
  const nextRef = useRef(null);
  const [swiperInstance, setSwiperInstance] = useState(null);
  const [allReviews, setAllReviews] = useState(defaultReviews); // ⬅️ Combined reviews
  ...
  useEffect(() => {
    const fetchDynamicReviews = async () => {
      try {
        ...
        setAllReviews([...defaultReviews, ...dynamicReviews]); // ⬅️ Merge static + dynamic
      } catch (error) {
